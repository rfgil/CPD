  ...
  for(i=0; i<6; i++){ //Vizita os 6 vizinhos de uma célula
    if (i != pov_caller){
      ...
      if (neighbor_cell != NULL){
        ...
        if (neighbor_cell->neighbors[POV[i]] == DEAD){
          return -1; //Significa que a célula viva já foi informada sobre a existência desta célula morta, logo esta posição do cubo já foi verificada
        }
