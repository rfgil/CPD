...
#define GET_PROCESSOR(index) ((index) < (BOT_COUNT * BOT_SIZE) ? (index) / BOT_COUNT : ((index) -  * BOT_COUNT) / TOP_COUNT + BOT_SIZE)
...
AVLTree ** LoadMap(char * file_name){
  ...
  // (o rank 0 analiza sempre BOT_SIZE células)
  map = calloc(BOT_SIZE + 2*SIZE, sizeof(AVLTree *));

  while(fscanf(input_file, "%d %d %d", &x, &y, &z) == 3){
    index = GET_INDEX(x, y);

    if (index < BOT_SIZE){
      // Esta célula pertence ao processador 0
      // No processador 0 o index está automaticamente normalizado
      if (map[index + SIZE] == NULL){ // Inicializa a AVL
      ...
      // Esta célula pertence a outro processador
      int MPI_Send(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm)
      buffer[0] = index;
      buffer[1] = z;

      MPI_Send(&buffer, 2, MPI_INT, GET_PROCESSOR(index), INIT_TAG, MPI_COMM_WORLD);
    }
  }
...
int main(int argc, char *argv[]){
  ...
  MPI_Barrier(MPI_COMM_WORLD); // Garante que todos os processadores têm a informação necessária para continuar

  if (!processor_id){
    // O processador 0 lê o mapa e distribui a informação
    processor_map = LoadMap(argv[1]);
  } else {
